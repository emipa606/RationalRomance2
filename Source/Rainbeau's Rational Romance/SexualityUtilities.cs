...
public static class SexualityUtilities
{
    ...
    public static Pawn FindAttractivePawn(Pawn p1)
    {
        ...
        if (LovePartnerRelationUtility.HasAnyLovePartner(pawn) && Rand.Value < 0.85f)
        {
            return null;
        }

        if (LovePartnerRelationUtility.ExistingLovePartners(p1).Any(relation => relation.otherPawn == pawn))
        {
            return null;
        }
